...
def data_sys_aifin(monggo_db: str, milvus_db: str, fenku: bool, status: int = 1):
    ...
    count = dbStore.countData({"status": {"$lt": status},'createTime':{'$lt':'2023-09-18T20:00:00'}})
    item_list = dbStore.listgroupCount({"status": {"$lt": status},'createTime':{'$lt':'2023-09-18T20:00:00'}},"code")
    ...
    for item_info in searchList:
        ...
        collects = [
            {"status": {"$lt": status},'createTime':{'$lt':'2023-09-18T20:00:00'}},
            {"code": item}
        ]
        query = {"$and": collects}
