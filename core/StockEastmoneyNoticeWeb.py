...
if __name__ == "__main__":
    ...
    if stockList and len(stockList) > 0:
        ...
        for stock in stockList:
            ...
            total,valid_data_total = east_notice(bMilvus, stock['stock_code'], stock['securities_name'], num, beginTime, endTime)
            countmap[stock['stock_code']] = f"一共{total}条数据，有效数据{valid_data_total}条"
            result_total += total
            result_valid_data_total += valid_data_total

    logger.info(f"本次脚本一共处理了{result_total}条数据，有效数据{result_valid_data_total}条，其中：{countmap}")
