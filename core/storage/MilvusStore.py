...
from config.common_config import store_milvus_status
from config.common_config import no_store_milvus_status
...
def storeData(input_docs: list[dict], collection_name: str, path: str = "47.97.218.138:31865"):
    # 定义每次获取的元素数量
    batch_size = 10
    store_list = []
    nostore_list = []
    ...
    for i in range(0, len(input_docs), batch_size):
        docs = input_docs[i:i + batch_size]
        # 在这里对每个批次的元素进行处理
        ...
        while True and err_count < 3:
            try:
                ...
                err_count += 1
        if obj:
            store_list.append(docs)
        else:
            nostore_list.append(docs)
    logger.info(f"写入矢量库【{collection_name}】over,正常入库{len(store_list)}条，异常{len(nostore_list)}条")
    return store_list, nostore_list


def storeMilvusTool(bMilvus: bool, input_docs: list[dict], collection_name: str, path: str = "47.97.218.138:31865"):
    logger.info("开始处理数据状态")
    result_total_list = []
    if input_docs and len(input_docs) > 0:
        if bMilvus:
            # 入矢量库
            store_list, nostore_list = storeData(input_docs, collection_name, path)
            # 设置状态
            if store_list and len(store_list) > 0:
                for doc in store_list:
                    doc.update({'status': store_milvus_status})
                result_total_list.append(store_list)
            if nostore_list and len(nostore_list) > 0:
                for doc1 in nostore_list:
                    doc1.update({'status': no_store_milvus_status})
                result_total_list.append(nostore_list)

        else:
            if input_docs and len(input_docs) > 0:
                for doc2 in input_docs:
                    doc2.update({'status': no_store_milvus_status})
                result_total_list.append(input_docs)
    logger.info("处理数据状态完成")
...
if __name__ == '__main__':
    ...
    dd = [{"source": "Web", "uniqueId": "20230904082200648218300", "code": "002377", "name": "国创高新",
           "url": "http://caifuhao.eastmoney.com/news/20230904082200648218300", "date": "2023-09-04 08:22:01",
           "type": "eastmoney-stock-carticle", "createTime": "2023-09-05 00:30:31",
           "abstract": "中介：当天二手房预约带看量增50%。（我爱我家、三六五网、国创高新、三湘印象、世联行等）5、电解铝：终端应用高增，1-8月国内电解铝表观需求达3.3%",
           "title": "9.4日｜发财哥早盘", "mediaName": "发财哥_1688",
    ...
    storeData(dd, f"aifin_stock_{1212}")
